...
- `h3` — Core H3 bindings (indexing, traversal, hierarchy, inspection, `<->` operator). No PostGIS dependency.
- `h3_postgis` — PostGIS integration (geometry/geography casts, `@` operator, polygon fill, raster helpers). Requires `postgis` and optionally `postgis_raster`.

**Decision rule:**
- IF you need `::geometry`, `::geography`, `h3_polygon_to_cells(geometry)`, or raster functions → install both extensions
- IF you only need cell math, grid traversal, or hierarchy operations → `h3` alone suffices

```sql
CREATE EXTENSION postgis;        -- if using spatial types
CREATE EXTENSION h3;
CREATE EXTENSION h3_postgis;     -- if using spatial types
```
...
- `h3_cell_to_boundary_geometry(h3index)` → polygon with SRID 4326
- `h3_cell_to_boundary_geography(h3index)` → geography polygon
- `h3_cell_to_boundary_wkb(h3index)` → EWKB, splits at antimeridian
- `h3_cells_to_multi_polygon_wkb(h3index[])` → EWKB multipolygon, splits at antimeridian
...
- `h3_get_resolution_from_tile_zoom(z, [max_h3_resolution], [min_h3_resolution], [hex_edge_pixels], [tile_size])` — pick H3 resolution for XYZ tiles
